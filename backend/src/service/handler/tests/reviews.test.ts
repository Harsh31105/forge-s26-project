...
import { CourseReview, ProfessorReview, ReviewPatchInputType } from "../../../models/review";
...
describe("ReviewHandler Endpoints", () => {
    ...
    describe("GET /reviews/:id", () => {
        test("returns course review by ID", async () => {
            ...
            const res = await request(app).get(`/reviews/${mockCourseReview.id}`);
            expect(res.status).toBe(200);
        ...
        test("returns professor review by ID", async () => {
            ...
            const res = await request(app).get(`/reviews/${mockProfReview.id}`);
            expect(res.status).toBe(200);
            expect(res.body).toMatchObject({
                id: "e4da3b7f-bbce-4a9b-9f0e-1c2f3a4b5c6d",
                profId: "c3d4e5f6-a7b8-4012-8def-123456789012",
        ...
        test("review not found returns 404", async () => {
            repo.getReviewByID.mockRejectedValue(new NotFoundError("review not found"));
            mockValidate.mockReturnValue(true);
            const res = await request(app).get(`/reviews/${mockCourseReview.id}`);
    ...
    describe("POST /reviews", () => {
        ...
        test("creates review without tags", async () => {
            ...
            const reviewNoTags: CourseReview = { ...mockCourseReview, tags: undefined };
            repo.createParentReview.mockResolvedValue(parentId);
        ...
        test("neither courseId nor profId provided returns 400", async () => {
            const payload = {
                rating: 4,
        ...
        test("repository throws foreign key error returns 400", async () => {
            repo.createParentReview.mockRejectedValue(new Error("foreign key constraint"));
    ...
    describe("PATCH /reviews/:id", () => {
        ...
        test("patches reviewText", async () => {
            const patch: ReviewPatchInputType = { reviewText: "Updated review text." };
            const updated: CourseReview = { ...mockCourseReview, reviewText: "Updated review text." };
            repo.patchReview.mockResolvedValue(updated);
            mockValidate.mockReturnValue(true);
        ...
        test("patches multiple fields", async () => {
            const patch: ReviewPatchInputType = { rating: 2, reviewText: "Changed my mind.", tags: ["hard"] };
            const updated: CourseReview = { ...mockCourseReview, rating: 2, reviewText: "Changed my mind.", tags: ["hard"] };
            repo.patchReview.mockResolvedValue(updated);
            mockValidate.mockReturnValue(true);
        ...
        test("censors profane reviewText in patch", async () => {
            const patch: ReviewPatchInputType = { reviewText: "This was a damn awful course" };
        ...
        test("patch with no fields succeeds", async () => {
            ...
            const res = await request(app).patch(`/reviews/${mockCourseReview.id}`).send({});
            expect(res.status).toBe(200);
        });

        test("invalid UUID returns 400", async () => {
            mockValidate.mockReturnValue(false);
            const res = await request(app).patch("/reviews/invalid-uuid").send({ rating: 3 });
            expect(res.status).toBe(400);
        });

        test("rating out of range returns 400", async () => {
            mockValidate.mockReturnValue(true);
            const res = await request(app).patch(`/reviews/${mockCourseReview.id}`).send({ rating: 10 });
            expect(res.status).toBe(400);
        });
        ...
        test("repository throws error returns 500", async () => {
            ...
            const res = await request(app).patch(`/reviews/${mockCourseReview.id}`).send({ rating: 3 });
            expect(res.status).toBe(500);
        });
    });
    ...
    describe("DELETE /reviews/:id", () => {
        test("deletes review", async () => {
            ...
            const res = await request(app).delete(`/reviews/${mockCourseReview.id}`);
            expect(res.status).toBe(204);
            expect(repo.deleteReview).toHaveBeenCalledWith(mockCourseReview.id);
        });
